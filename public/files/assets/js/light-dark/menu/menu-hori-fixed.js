...
$( document ).ready(function() {
    ...
    handlenavimg();
    /* Backend type Change function Start */
    function handlelayouttheme() {
        $('.theme-color > a.Layout-type').on("click", function () {
            var layout = $(this).attr("Backend-type");
            $('.pcoded').attr("Backend-type", layout);
            if(layout=='dark'){
    ...
    function handlethemeverticallayout() {
        $('#theme-Backend').change(function () {
            if ($(this).is(":checked")) {
                $('.pcoded').attr('vertical-Backend', "box");
                $('#bg-pattern-visiblity').removeClass('d-none');

            } else {
                $('.pcoded').attr('vertical-Backend', "wide");
